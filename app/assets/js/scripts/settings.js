        ...
        val.onclick = (e) => {
            ...
            if(Object.keys(ConfigManager.getAuthAccounts()).length === 1){
                ...
                setOverlayContent(
                    ...
                    'Pour utiliser le ZendLauncher, vous devez être connecté à au moins un compte. Vous devrez vous reconnecter après.<br><br>Êtes-vous sûr de vouloir vous déconnecter ?',
                    'Je suis sûr',
                    'Annuler'
                )
                setOverlayHandler(() => {
...
ipcRenderer.on(MSFT_OPCODE.REPLY_LOGOUT, (_, ...arguments_) => {
    if (arguments_[0] === MSFT_REPLY_TYPE.ERROR) {
        switchView(getCurrentView(), VIEWS.settings, 500, 500, () => {
            ...
            setOverlayContent(
                'Quelque chose s\'est mal passé',
                'La déconnexion avec Microsoft a échoué. Veuillez réessayer.',
                'OK'
            )
            setOverlayHandler(() => {
                toggleOverlay(false)
        ...
        const accHtml = `<div class="settingsAuthAccount" uuid="${acc.uuid}">
            ...
            <div class="settingsAuthAccountRight">
                ...
                <div class="settingsAuthAccountActions">
                    <button class="settingsAuthAccountSelect" ${selectedUUID === acc.uuid ? 'selected>Compte sélectionnez &#10004;' : '>Select Account'}</button>
    ...
    Array.from(sEls).map((v, index, arr) => {
        v.onclick = async () => {
            ...
            if(res){
                ...
                setOverlayContent(
                    `Échec de la suppression d'un<br>Drop-in Mod ${fullName}`,
        ...
        <div class="serverListingDetails">
            ...
            <div class="serverListingInfo">
                ...
                ${serv.isMainServer() ? `<div class="serverListingStarWrapper">
                    ...
                    <span class="serverListingStarTooltip">Serveur Principale</span>
                </div>` : ''}
            </div>
        </div>
    `
    ...
    jg._validateJavaBinary(execPath).then(v => {
        if(v.valid){
            ...
            if(v.version.major < 9) {
                settingsJavaExecDetails.innerHTML = `Selectionner : Java ${v.version.major} Mise à jour ${v.version.update} (x${v.arch})${vendor}`
            } else {
                settingsJavaExecDetails.innerHTML = `Selectionner : Java ${v.version.major}.${v.version.minor}.${v.version.revision} (x${v.arch})${vendor}`
            }
        } else {
...
function populateVersionInformation(version, valueElement, titleElement, checkElement){
    ...
    if(isPrerelease(version)){
        titleElement.innerHTML = 'Version Préliminaire'
...
function populateReleaseNotes(){
    $.ajax({
        url: 'https://github.com/zendrique/ZendLauncher/releases.atom',
        success: (data) => {
...
function populateSettingsUpdateInformation(data){
    if(data != null){
        settingsUpdateTitle.innerHTML = `Nouvelle ${isPrerelease(data.version) ? 'version Préliminaire' : 'Version'} Disponible`
        ...
        if(process.platform === 'darwin'){
            settingsUpdateButtonStatus('Télécharger depuis GitHub<span style="font-size: 10px;color: gray;text-shadow: none !important;">Fermez le ZendLauncher et exécutez le dmg pour mettre à jour.</span>', false, () => {
        ...
        settingsUpdateButtonStatus('Recherche de mise à jour', false, () => {
            if(!isDev){
